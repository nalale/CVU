...
namespace FTDI_CAN
{
	...
	public class UCan : ICanDriver
	{
		...
		public event EventHandler<CAN_Message> OnSysMessage;
...
//		bool txThreadAlive = true;
		...
		public bool Start(int channel, int bitRate)
		{
			...
			if (!(CheckTask?.Status == TaskStatus.Running))
			{
				...
				CheckTask = Task.Factory.StartNew(check_thread, cancelSource.Token);
			}
		...
		void Connect()
		{
            ...
            {
                ftStatus = ftdiDevice.OpenBySerialNumber(ftdiDeviceList[i].SerialNumber);  
                ...
                FTDI.FT_STATUS st = ftdiDevice.GetDeviceType(ref dev);

                // DE CAN adapter use this device type
                if (dev == FTDI.FT_DEVICE.FT_DEVICE_232R)
                {
					...
					if(!(RxTask?.Status == TaskStatus.Running))
						RxTask = Task.Factory.StartNew(recieve_thread, cancelSource.Token);

					if (!(TxTask?.Status == TaskStatus.Running))
						TxTask = Task.Factory.StartNew(send_thread, cancelSource.Token);
		...
		public bool SetSpeed(int bitRate)
		{
			...
			if (ftStatus != FTDI.FT_STATUS.FT_OK)
			{
				throw new Exception(string.Format("Failed to write to ftdi. Actual ftdi status:/n{0}", ftStatus));
		...
		public bool SendMessage(ICanMessage msg)
		{
			...
			if (msg.Ext == false)
			{
				...
				foreach(int i in msg.data)
				{
					string sub_str = i.ToString("X2");
					s += sub_str;
				}
			...
			else
			{
				...
				if (msg.id == 0x18DA17F1 || msg.id == 0x18DAF117)
					OnSysMessage?.Invoke(this, (CAN_Message)msg);
